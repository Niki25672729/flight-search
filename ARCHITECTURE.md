...
A Python CLI tool that searches for budget flights from a European departure airport within a given time range and budget. It scrapes budget airline websites, caches results locally for 1 week, and displays results as a table. A web interface is also planned for a future version.
...
| Field          | Value                                                          |
|----------------|----------------------------------------------------------------|
| Responsibility | Shared helpers — load EU airport data and unknown airports     |
| Inputs         | None                                                           |
| Outputs        | `EU_AIRPORT_DETAILS`, `EU_AIRPORTS`, `unknown_airport_details` |
| Key files      | `utils.py`                                                     |
| External calls | Local filesystem — `eu_airports.json`, `unknown_airports.json` |
...
| External calls | Ryanair (v1); easyJet, Wizz Air, Vueling, Norwegian, Eurowings (planned) |
...
8. `flight_search.py` filters flights by time range and budget, sorts by country, city, and time
...
|------------|---------|--------------------------------------|-------------------------------------------|
| ryanair-py | 3.0.0   | Ryanair flight data via ryanair-py   | https://github.com/cohaolain/ryanair-py   |
...
- Ryanair session cookie fetch takes ~10 minutes; cached locally for 1 hour to avoid repeated slow initialisation
...
|-----------|-----------------|-----------------|--------------|
| Ryanair   | ryanair.com     | ryanair-py      | ✅ v1        |
...
|--------------------|--------------------------------------------------|
| Latency            | Scraping may take ~10 min for first run (cookie fetch); subsequent runs use cache |
| Cache TTL          | 1 week                                           |
